...
{
   if (argc < 6) {
    cerr << "Please give 6 arguments " << "runList " << " " << "outputFileName" << " " << "dataset" <<" " << "configuration" 
	 <<" " << "energy" << " " <<"event_cat"
	 << " " << "FH_noise"<<endl;
    return -1;
  }
...
void AnalyzeHGCOctTB::EventLoop(const char *data, bool UseSSBasedCat, int noise_FH_) {
  ...
  bool ScaleSim = true;
  // float ee_rescaling = 1.08;
  // //float fh_rescaling = 1.15;
  // float fh_rescaling = 1.15;
  // float ah_rescaling = 1.04;
  ...
  for (jentry=0; jentry<nentries;jentry++) {
    ...
    // MUON VETO
    if((S25_AH > 0.0 && S1_AH/S25_AH > 0.80) && (rechitEnergySum_EE < 100) && (rechitEnergySum_FH < 60)) {
      MIP = true;
    ...
    // cout<<"yolo => "<<(S1_AH/S25_AH)<<" : "<<rechitEnergySum_EE<<" : "<<rechitEnergySum_FH<< " ==> "<<MIP<<endl;
    double total_energy = total_energy_EE + total_energy_FH + total_energy_AH;
    // double total_energy = total_energy_EE + total_energy_FH ;
