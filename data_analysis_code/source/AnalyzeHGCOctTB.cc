...
void AnalyzeHGCOctTB::EventLoop(const char *data, bool UseSSBasedCat, int noise_FH_) {
  ...
  for (jentry=0; jentry<nentries;jentry++,hgc_jentry++) {
    ...
    // if(DEBUG) cout << "SW >>>>>> Starting of HGCAL loop, Took: "; sw.Print("u"); 
    ...
    // if(DEBUG) cout << "SW >>>>>>END of AHCAL loop; Took: "; sw.Print(); 
	...
	total_energy_EE += RechitsEn_EE[iL];
	...
	total_energy_FH += RechitsEn_FH[iL-EE_LAYER];
	...
	E19_FH += E19;
      ...
      if(iL < 39) {
	total_energy_AH += RechitsEn_AH[iL];
    ...
    // if(DEBUG) cout << "SW >>>>>>END of compartmentwise cal loop; Took: "; sw.Print(); 
    ...
    // if(DEBUG) cout << "SW >>>>>>END of new SS finder algo loop; Took: "; sw.Print(); 
    ...
    // 	if(iL < EE_LAYER) {
    // 	  // if(!((iL+1 == 6) || (iL+1 == 10) || (iL+1 == 18) || (iL+1 == 24) || (iL+1 == 28))) continue;
    ...
    // cout<<" ******************************** "<<endl;
    // cout<< " Event : "<<jentry<< " ::  seed-size "<<seed_xy.size()<<endl;
    
    /////// yolo spandey here look at me
    ...
    // if(DEBUG) cout << "SW >>>>>>END of longi shower profile calc; Took: "; sw.Print(); 
    ...
    if(isRegion3) {
      ...
      bool IsIn_en_range = (total_energy > en_range_EE_xmin[(int)beamEnergy] && total_energy < en_range_EE_xmax[(int)beamEnergy]);
