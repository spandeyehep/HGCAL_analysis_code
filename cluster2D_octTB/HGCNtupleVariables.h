...
public :
  ...
  std::vector<RecHit*> makeCluster(std::vector<RecHit*> rechits_1);
  float R_ij(float ui_, float vi_, float uj_, float vj_);
  std::vector<RecHit*> sort_rechits(std::vector<RecHit*> rechits_);
  std::vector<Cluster2D*> ClusterProducer(std::vector<RecHit*> rechits_coll_);
