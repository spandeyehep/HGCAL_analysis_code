...
void AnalyzeHGCOctTB::EventLoop(const char *data) {
  ...
  for (jentry=0; jentry<nentries;jentry++) {
    ...
    // case 20: a = 0.0183585; b = 0.0697434; c = 0.0389642; d = 0.0662263; d_prime = 0.0772711; break;  // => Default
    // case 20: a = 0.0183585; b = 0.0697434; c = 0.0389642; d = 0.0662263; d_prime = 0.105706; break;  // => newChi2 only D updated
    // case 20: a = 0.0205465; b = 0.077439; c = 0.0389642; d = 0.0662263; d_prime = 0.0914029; break;   // => newChi2_v1 only ABD updated
    case 20: a = 0.0235866; b = 0.0828442; c = 0.0389642; d = 0.0662263; d_prime = 0.106934; break;   // => newChi2_v1 only ABD updated
    case 50: a = 0.0169303; b = 0.0865168; c = 0.064306 ;d = 0.0766219;  d_prime = 0.0990474;break;
    case 80: a = 0.0161748 ;b = 0.0901873; c = 0.0929755 ;d = 0.0784754; d_prime = 0.0963385;break;
    case 100: a = 0.0158182 ;b = 0.0909478; c = 0.106462 ;d = 0.0792468; d_prime = 0.0950571;break;
    case 120: a = 0.0155922 ;b = 0.0902639; c = 0.103944 ;d = 0.0817746; d_prime = 0.0957698;break;
    case 200: a = 0.0150454 ;b = 0.092911; c = 0.158882 ;d = 0.0837074; d_prime = 0.0942076;break;
    case 250: a = 0.0149132 ;b = 0.0929005; c = 0.207688 ;d = 0.0833859; d_prime = 0.0959323;break;
      //case 300:  a = 0.0148512 ;b = 0.0934919; c = 0.226683 ;d = 0.0849056; d_prime =  0.0978152;break;
    case 300:  a = 0.0161207 ;b = 0.0951512; c = 0.226683 ;d = 0.0849056; d_prime =  0.104317;break;
    default: cout<<"undefined beamEnergy!!!"<<endl; a = 0.0; b = 0.0; c = 0.0; d = 0.0; d_prime = 0.0;return; 

    }
    double weighted_energy_all = -1.0;
    double weighted_energy_EE = -1.0;
    double weighted_energy_FH = -1.0;
    double isMuonLike = false;
    h_energy_EEFH->Fill(rechitEnergySum_EE+rechitEnergySum_FH);
    if(Nrechit_EE < 80 && Nrechit_FH < 60)  { // MIP in both
      h_energy_EEFH_MIPEEFH->Fill(rechitEnergySum_EE+rechitEnergySum_FH);
      h_energy_EE_MIPEEFH->Fill(rechitEnergySum_EE);
      h_energy_FH_MIPEEFH->Fill(rechitEnergySum_FH);
      h_energy_ratio_EEFH_MIPEEFH->Fill(rechitEnergySum_EE/rechitEnergySum_FH);
      if(rechitEnergySum_EE/rechitEnergySum_FH < 0.6) {	
	h_Nrechit_EE_R1->Fill(Nrechit_EE);
	h_Nrechit_FH_R1->Fill(Nrechit_FH);
	h_energy_EE_R1->Fill(rechitEnergySum_EE);
	h_energy_FH_R1->Fill(rechitEnergySum_FH);

      }
      else if(rechitEnergySum_EE/rechitEnergySum_FH > 0.6 && rechitEnergySum_EE/rechitEnergySum_FH < 2.0) {
	h_Nrechit_EE_R2->Fill(Nrechit_EE);
	h_Nrechit_FH_R2->Fill(Nrechit_FH);
	h_energy_EE_R2->Fill(rechitEnergySum_EE);
	h_energy_FH_R2->Fill(rechitEnergySum_FH);

      }
      else {
	h_Nrechit_EE_R3->Fill(Nrechit_EE);
	h_Nrechit_FH_R3->Fill(Nrechit_FH);
	h_energy_EE_R3->Fill(rechitEnergySum_EE);
	h_energy_FH_R3->Fill(rechitEnergySum_FH);

      }
      h_energy_EEFH_MIPEEFH_2D->Fill(rechitEnergySum_EE,rechitEnergySum_FH);
      if(rechitEnergySum_EE > 120.0) {

	h_Nrechit_vs_energy_EEFH_MIPEEFH_2D->Fill(Nrechit_EE,rechitEnergySum_EE);
      }
      isMuonLike = true;
    }
    else if(Nrechit_EE < 80) {    // H - hadrons
      h_energy_EEFH_MIPEE->Fill(rechitEnergySum_EE+rechitEnergySum_FH);
      h_energy_EE_MIPEE->Fill(rechitEnergySum_EE);
      h_energy_FH_MIPEE->Fill(rechitEnergySum_FH);
      h_energy_EEFH_MIPEE_2D->Fill(rechitEnergySum_EE,rechitEnergySum_FH);
      h_energy_H_hadrons_cd->Fill(c*rechitEnergySum_EE+d*rechitEnergySum_FH);
      h_energy_H_hadrons_d->Fill(d_prime*rechitEnergySum_FH);
      h_energy_H_hadrons_cd_2D->Fill(c*rechitEnergySum_EE,d*rechitEnergySum_FH);
      h_NrechitEE_vs_energyEE_MIPEE_2D->Fill(Nrechit_EE,rechitEnergySum_EE);
      weighted_energy_EE = c*rechitEnergySum_EE;
      weighted_energy_FH = d*rechitEnergySum_FH;
      weighted_energy_all = weighted_energy_EE+weighted_energy_FH;
    }
    else {   // EH - hadrons
      h_energy_EEFH_EH_hadrons->Fill(rechitEnergySum_EE+rechitEnergySum_FH);
      h_energy_EEFH_EH_hadrons_2D->Fill(rechitEnergySum_EE,rechitEnergySum_FH);
      h_energy_EH_hadrons_ab->Fill(a*rechitEnergySum_EE+b*rechitEnergySum_FH);
      h_energy_EH_hadrons_ab_2D->Fill(a*rechitEnergySum_EE,b*rechitEnergySum_FH);
      weighted_energy_EE = a*rechitEnergySum_EE;
      weighted_energy_FH = b*rechitEnergySum_FH;
      weighted_energy_all = weighted_energy_EE+weighted_energy_FH;
    }
    if(!isMuonLike) {
      h_energy_all_hadrons_all->Fill(weighted_energy_all);
      h_energy_all_hadrons_all_2D->Fill(weighted_energy_EE,weighted_energy_FH);
    }
